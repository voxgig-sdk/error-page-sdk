...
module ErrorPageUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ErrorPageResponse.new({})
    ...
    if fetch_err
      ...
      response = ErrorPageResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ErrorPageResponse.new(fetched)
    else
