...
require_relative 'ErrorPage_types'
...
class ErrorPageSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ErrorPageHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ErrorPageHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ErrorPageFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ErrorPageUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ErrorPageHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ErrorPageHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ErrorPageHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ErrorPageHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ErrorPageError.new(
        "graphql_error", "ErrorPageSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.TechnologyDetection.list / client.TechnologyDetection.load({ "id" => ... })
  def TechnologyDetection(data = nil)
    require_relative 'entity/technology_detection_entity'
    TechnologyDetectionEntity.new(self, data)
  end
