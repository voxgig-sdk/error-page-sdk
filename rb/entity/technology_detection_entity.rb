...
class TechnologyDetectionEntity
  ...
  def make
    opts = @_entopts.dup
    TechnologyDetectionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ErrorPageHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TechnologyDetection, Hash] the current TechnologyDetection data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ErrorPageHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TechnologyDetection fields)
  def match_get
  ...
  #
  # @param reqmatch [TechnologyDetectionListMatch, Hash, nil] match filter (any subset of
  #   TechnologyDetection fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TechnologyDetection>, Array] the matching TechnologyDetection items; raises ErrorPageError on failure
