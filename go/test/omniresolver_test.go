...
import (
	...
	omni "github.com/voxgig-sdk/error-page-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ErrorPageError); is {
				out := map[string]any{
					"name":    "ErrorPageError",
...
func omniCtx(arg any, client *sdk.ErrorPageSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ErrorPageSDK); is {
			client = live
