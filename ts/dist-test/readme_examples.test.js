...
const TEST_SEED = { "entity": { "technology_detection": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ErrorPageSDK\b/.test(code) ||
        /\bErrorPageSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ErrorPageSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ErrorPageSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
