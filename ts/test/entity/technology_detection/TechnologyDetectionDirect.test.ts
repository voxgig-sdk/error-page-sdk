...
describe('TechnologyDetectionDirect', async () => {
  ...
  test('direct-list-technology_detection', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-technology_detection', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ERROR_PAGE_TEST_TECHNOLOGY_DETECTION_ENTID']
