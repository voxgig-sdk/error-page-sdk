...
local TechnologyDetectionEntity = {}
TechnologyDetectionEntity.__index = TechnologyDetectionEntity


function TechnologyDetectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TechnologyDetectionEntity)
  self._name = "technology_detection"
  self._client = client
...
function TechnologyDetectionEntity:get_name()
  return self._name
end


function TechnologyDetectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TechnologyDetectionEntity.new(self._client, opts)
end
...
function TechnologyDetectionEntity:mark_deleted()
  self._deleted = true
end


function TechnologyDetectionEntity:deleted()
  return true == self._deleted
end


function TechnologyDetectionEntity:data_set(args)
  if args ~= nil then
...
function TechnologyDetectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TechnologyDetectionEntity:match_set(args)
  if args ~= nil then
...
function TechnologyDetectionEntity:match_get()
...
function TechnologyDetectionEntity:stream(action, args, callopts)
...
---@param reqmatch TechnologyDetectionListMatch
---@param ctrl? table
---@return TechnologyDetection[]
---@return string? err
function TechnologyDetectionEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TechnologyDetectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return TechnologyDetectionEntity
