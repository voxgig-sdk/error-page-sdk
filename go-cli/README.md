...
boru-driven command-line client **and** interactive REPL for the ErrorPage
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/error-page-cli)
make build
...
./error-page-cli list technology_detection

# 5. Override the API base URL for a single call
ERROR_PAGE_BASE=https://api.example.com ./error-page-cli list technology_detection

# 6. No arguments -> interactive REPL
./error-page-cli
error-page> list technology_detection
error-page> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/error-page-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/error-page-cli list technology_detection
   ```
...
Run with no arguments to open a REPL (prompt `error-page>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `technology_detection`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/error-page-cli`. |
...
(`./error-page-cli <expr>`) and interactively (the REPL), and expressions compose the
