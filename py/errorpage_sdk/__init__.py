...
from errorpage_sdk.utility import register

# Load features
from errorpage_sdk.feature.base_feature import ErrorPageBaseFeature
from errorpage_sdk.features import _has_feature, _make_feature
...
class ErrorPageSDK:
    ...
    def get_utility(self):
        return ErrorPageUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("ErrorPageSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def TechnologyDetection(self, data=None) -> "TechnologyDetectionEntity":
        """Entity factory: client.TechnologyDetection().list() / client.TechnologyDetection().load({"id": ...})."""
        from errorpage_sdk.entity.technology_detection_entity import TechnologyDetectionEntity
        return TechnologyDetectionEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "ErrorPageSDK":
        if sdkopts is None:
            sdkopts = {}
