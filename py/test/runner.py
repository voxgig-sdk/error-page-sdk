...
class ErrorPageTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ErrorPageTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ErrorPageTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ErrorPageTestRunner.getenv("ERROR_PAGE_TEST_LIVE")
        override = ErrorPageTestRunner.getenv("ERROR_PAGE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ErrorPageTestRunner.getenv(key)
        ...
        explain = ErrorPageTestRunner.getenv("ERROR_PAGE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ERROR_PAGE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ErrorPageTestRunner._test_control is not None:
            return ErrorPageTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ErrorPageTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ErrorPageTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ErrorPageTestRunner.load_test_control()
...
def load_env_local():
    ErrorPageTestRunner.load_env_local()


def env_override(m):
    return ErrorPageTestRunner.env_override(m)


def entity_data(v):
    return ErrorPageTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ErrorPageTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ErrorPageTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ErrorPageTestRunner.load_test_control()


def live_client_options():
    return ErrorPageTestRunner.live_client_options()


def live_delay_ms():
    return ErrorPageTestRunner.live_delay_ms()
