...
class ErrorPageRetryFeature extends ErrorPageBaseFeature
{
    ...
    public function init(ErrorPageContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ErrorPageContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ErrorPageContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ErrorPageContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
