...
class FtCtrl extends ErrorPageControl
{
...
class FtClient
{
    ...
    private ErrorPageUtility $utility;

    public function __construct(ErrorPageUtility $utility, array $options)
    {
    ...
    public function get_utility(): ErrorPageUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ErrorPageContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ErrorPageContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ErrorPageContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ErrorPageConfig::make_feature($name);
    ...
    private static function buildUrl(ErrorPageSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ErrorPageError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ErrorPageResponse($fetched);
        }
    ...
    private function fail(ErrorPageContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ErrorPageContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
