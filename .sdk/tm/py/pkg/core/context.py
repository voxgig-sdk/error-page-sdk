...
from projectname_sdk.core.control import ErrorPageControl
...
class ErrorPageContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, ErrorPageSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ErrorPageResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ErrorPageResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return ErrorPageError(code, msg, self)
