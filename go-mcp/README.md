...
[MCP](https://modelcontextprotocol.io) server exposing the ErrorPage SDK as
two agent tools — `error-page_list` and `error-page_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/error-page-mcp)
make build

# 2. Provide credentials via the environment
export ERROR_PAGE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user error-page \
  -- /absolute/path/to/error-page-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./error-page-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// error-page_list: first page of records
{ "entity": "technology_detection" }
{ "entity": "technology_detection", "query": { } }

// error-page_load: one record by id
{ "entity": "technology_detection", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/error-page-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user error-page \
     -- "$PWD"/dist/*/error-page-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `error-page_list` and `error-page_load` tools now appear
   in new sessions. Ask the agent to *"list technology_detection using error-page"*
   and it calls `error-page_list` with `{"entity":"technology_detection"}`.
...
```sh
export ERROR_PAGE_APIKEY=sk_live_xxx            # API key
export ERROR_PAGE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `error-page_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `error-page_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ERROR_PAGE_APIKEY` | API key sent with every request. |
| `ERROR_PAGE_BASE` | Optional override of the API base URL. |
...
```sh
./error-page-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"error-page_load","arguments":{"entity":"technology_detection","query":{"id":1}}}}'
```
