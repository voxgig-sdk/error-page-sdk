...
type Args struct {
	Entity string         `json:"entity" jsonschema:"technology_detection"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ErrorPageSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "error-page_list",
		Description: "List records from ErrorPage. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "error-page_load",
		Description: "Load a single record from ErrorPage. " +
...
func runOp(client *sdk.ErrorPageSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ErrorPageSDK, name string) (sdk.ErrorPageEntity, error) {
	switch strings.ToLower(name) {
	case "technology_detection":
		return client.TechnologyDetection(nil), nil
