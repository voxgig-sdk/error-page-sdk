...
// ErrorPage SDK TechnologyDetection entity
...
class TechnologyDetectionEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "technology_detection";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new TechnologyDetectionEntity($this->_client, $opts);
    }

    /**
     * @param TechnologyDetection|array $args TechnologyDetection data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ErrorPageHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TechnologyDetection|array The current TechnologyDetection data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TechnologyDetection fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ErrorPageHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TechnologyDetection fields).
     */
    ...
    /**
     * List TechnologyDetection items matching the given filter.
     *
     * @param TechnologyDetectionListMatch|array|null $reqmatch Match filter (any subset
     *   of TechnologyDetection fields) as an assoc-array; TechnologyDetectionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TechnologyDetection[]|array A list of TechnologyDetection items as assoc-arrays at
     *   the SDK boundary; throws ErrorPageError on failure (item-5 convention).
     */
