...
class ErrorPageMakeError
{
    public static function call(?ErrorPageContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ErrorPageContext([], null);
        }
        $op = $ctx->op ?? new ErrorPageOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ErrorPageResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ErrorPageError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ErrorPageError('', $msg, $ctx);
