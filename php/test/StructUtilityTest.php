...
// ErrorPage SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ErrorPageOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ErrorPageOmni::structfix($now, Struct::undef()))
                        !== json_encode(ErrorPageOmni::structfix($val, Struct::undef()))) {
